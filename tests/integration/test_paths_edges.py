...
class TestPathsEdges(EdgesSettings):
    ...
    def test_path1(self):
        ...
        for point_a, point_b in combos:
            results = self.graph.k_shortest_paths(point_a, point_b)
            if not results:
                valid = False
                break
    ...
    def test_path2(self):
        ...
        for point_a, point_b in combos:
            results = self.graph.constrained_k_shortest_paths(point_a, point_b)
    ...
    def test_path10(self):
        """Tests that TypeError is generated by self.graph.constrained_k_shortest_paths
